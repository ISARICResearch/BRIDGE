...
class Language:
    ...
    def get_version_language_related_data(self):
        df_version, version_presets, version_commit = arc.getARC(self.selected_version)
        df_version = arc.add_required_datadicc_columns(df_version)
        df_version_language = self.get_dataframe_arc_language(df_version)

        version_arc_lists, version_list_variable_choices = arc.getListContent(df_version_language,
                                                                              self.selected_version,
                                                                              self.selected_language)
        df_version_language = arc.addTransformedRows(df_version_language, version_arc_lists,
                                                     arc.getVariableOrder(df_version_language))

        version_arc_ulist, version_ulist_variable_choices = arc.getUserListContent(df_version_language,
                                                                                   self.selected_version,
                                                                                   self.selected_language)
