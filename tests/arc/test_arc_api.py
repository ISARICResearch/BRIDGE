...
@mock.patch('bridge.arc.arc_api.ArcApiClient._get_api_response')
def test_get_arc_version_list_development(mock_release_json,
                                          client_development):
    release_json = [
        {'_links': {'git': 'https://api...'}, 'download_url': None, 'name': 'ARCH0.0.1', 'path': 'ARCH/ARCH0.0.1'},
        {'_links': {'git': 'https://api...'}, 'download_url': None, 'name': 'ARCH1.1.0', 'path': 'ARCH/ARCH1.1.0'},
        {'_links': {'git': 'https://api...'}, 'download_url': None, 'name': 'ARCH1.1.1', 'path': 'ARCH/ARCH1.1.1'},
        {'_links': {'git': 'https://api...'}, 'download_url': None, 'name': 'ARCH1.1.2', 'path': 'ARCH/ARCH1.1.2'},
        {'_links': {'git': 'https://api...'}, 'download_url': None, 'name': 'ARCH1.1.3', 'path': 'ARCH/ARCH1.1.3'},
        {'_links': {'git': 'https://api...'}, 'download_url': None, 'name': 'ARCH1.1.4', 'path': 'ARCH/ARCH1.1.4'},
        {'_links': {'git': 'https://api...'}, 'download_url': None, 'name': 'ARCH1.2.0-rc', 'path': 'ARCH/ARCH1.2.0-rc'}
    ]
...
@mock.patch('bridge.arc.arc_api.ArcApiClient._get_api_response')
def test_get_arc_version_sha_development(mock_tag_json,
                                         client_development):
    expected_sha = '000000a'
    tag_json = [
        {'message': 'Update ARCH.csv definition', 'tree': {'sha': '000000'}, 'sha': '000000a'},
        {'message': 'Update something else', 'tree': {'sha': '111111'}, 'sha': '111111a'},
        {'message': 'Another older update', 'tree': {'sha': '222222'}, 'sha': '222222a'},
    ]
    mock_tag_json.return_value = tag_json
...
def test_get_dataframe_arc_list_version_language(mock_write_to_df,
    ...
    df_output = client_production.get_dataframe_arc_list_version_language(version,
                                                                          language,
                                                                          list_name)
...
@mock.patch('bridge.arc.arc_api.ArcApiClient._write_to_dataframe')
def test_get_dataframe_arc_list_version_language_development(mock_write_to_df,
                                                             client_development):
    data_list = {
        'Condition': [
            'Obesity',
    ...
    language = 'English'
    list_name = 'conditions/Comorbidities'
    df_output = client_development.get_dataframe_arc_list_version_language(version,
                                                                           language,
                                                                           list_name)
