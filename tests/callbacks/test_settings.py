...
def test_update_output_files_store():
    ...
    checked_values = ["redcap_xml", "redcap_csv", "paper_like"]
...
@mock.patch(
    "bridge.callbacks.settings.dbc.DropdownMenuItem",
    return_value="Mock DropdownMenuItem",
)
@mock.patch(
    "bridge.callbacks.settings.ArcApiClient.get_arc_language_list_version",
...
def test_store_data_for_selected_version_language_no_action(
    trigger,
    clicks_version,
    clicks_language,
    crf_ready,
...
@mock.patch(
    "bridge.callbacks.settings.arc_core.get_arc_versions",
    return_value=(["v1.1.0", "v1.1.1", "v1.1.2"], "v1.1.2"),
)
@mock.patch(
    "bridge.callbacks.settings.get_trigger_id",
    return_value='{"index":0,"type":"dynamic-version"}',
)
...
@mock.patch(
    "bridge.callbacks.settings.get_trigger_id",
    return_value='{"index":1,"type":"dynamic-language"}',
)
@mock.patch("bridge.callbacks.settings.logger")
