...
def test_update_output_based_on_url_no_action(template_check, presets, address, expected_output):
...
@pytest.mark.parametrize(
    ...
    [
        (
                True,
                {'ARChetype Disease CRF': ['Covid', 'Dengue', 'Mpox'], 'ARChetype Syndromic CRF': ['ARI']},
                'http://127.0.0.1:8050/main?param=ARChetype%20Disease%20CRF_Dengue',
                (['Dengue'], [['Dengue'], []]),
        ),
        (
                True,
                {'ARChetype Disease CRF': ['Covid', 'Dengue', 'Mpox'], 'Recommended Outcomes': ['Dengue']},
...
def get_output_update_output_based_on_url(template_check, presets, address):
    def run_callback(template_check_flag,
                     grouped_presets,
                     href):
        return url.update_output_based_on_url(template_check_flag,
                                              grouped_presets,
                                              href)
