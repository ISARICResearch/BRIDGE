...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "template_test_v1_1_2_English_2025-09-16.csv",
            None,
            (
                {"upload_version": "v1.1.2"},
                {"upload_language": "English"},
            ),
        ),
    ],
)
...
def test_update_list_variables_checked_upload(_mock_get_translations):
    ...
    data_list = {
        ...
        "Ulist Selected": [
            "Adenovirus|Andes virus infection (hantavirus)",
            "Aland Islands",
        ],
    ...
    data = {
        ...
        "Variable": ["inclu_disease", "demog_country", "inclu_disease_otherl2"],
        "Answer Options": [
            "1, Adenovirus | 21, Andes virus infection (hantavirus) | 88, Other",
...
def test_load_upload_arc_version_language_not_triggered():
    ...
    output = get_output_load_upload_arc_version_language(
        trigger,
        upload_version_data,
...
def triggered_trigger():
    trigger = [
        {"prop_id": "upload-version-store.data", "value": {"upload_version": "v1.1.2"}},
        {
            "prop_id": "upload-language-store.data",
            "value": {"upload_language": "English"},
        },
    ]
    return trigger
...
def get_output_load_upload_arc_version_language(
    trigger,
    upload_version_data,
    upload_language_data,
    selected_version_data,
    ...
    def run_callback():
        ...
        return upload.load_upload_arc_version_language(
            upload_version_data,
            upload_language_data,
            selected_version_data,
            selected_language_data,
...
def test_update_output_upload_crf(_mock_get_tree_items, mock_update, _mock_html_div):
    ...
    upload_language_data = {"upload_language": "English"}
    upload_crf_contents = (
        "data:text/csv;base64,VmFyaWFibGUsVWxpc3QgU2VsZWN0ZWQsTXVsdGlsaXN0IFNlbGVjdGVkCmRlbW9nX2Nvd"
...
def get_output_update_output_upload_crf(
    trigger,
    upload_crf_ready,
    upload_version_data,
    upload_language_data,
    ...
    def run_callback():
        context_value.set(AttributeDict(**{"triggered_inputs": trigger}))
        return upload.update_output_upload_crf(
            upload_crf_ready,
            upload_version_data,
            upload_language_data,
            upload_crf_contents,
