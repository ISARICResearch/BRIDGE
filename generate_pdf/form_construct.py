...
from generate_pdf.form_classes import Field, SubsectionStyle, Row, Subsection, Subsubsection
...
def construct_standard_row(
    ...
    if style == SubsectionStyle.HEADING:
    ...
    if row.is_shaded:
        style = SubsectionStyle.QA_BOARDERLESS
    ...
    if bool(subsubsection.conditional_text != None):
        style = SubsectionStyle.QA_BOARDERLESS
        if row_index == 0 and subsub_index > 1:
            table_style.add('LINEABOVE', (1, 0), (-2, -1), line_width, grey_80)
            table_style.add('LINEBELOW', (1, 0), (-2, -1), line_width, grey_80)
    ...
    if style == SubsectionStyle.QA_GREY:
        # Handle left & right borders
        if len(row.fields) == 3:
            for i in range(row_length-2):
                x=i*2
                table_style.add('LINEBEFORE', (x+1, 0), (x+1, -1), line_width, grey_80) # should depend on row_length
        elif len(row.fields) == 2:
            table_style.add('LINEBEFORE', (int(row_length/2), 0), (int(row_length/2), -1), line_width, grey_80) # should depend on row_length
    ...
    if style == SubsectionStyle.QA_BLACK:
        # Handle left & right borders
        if len(row.fields) == 3:
            for i in range(row_length-2):
                x=i*2
                table_style.add('LINEBEFORE', (x+1, 0), (x+1, -1), line_width, colors.black) # should depend on row_length
        elif len(row.fields) == 2:
            table_style.add('LINEBEFORE', (int(row_length/2), 0), (int(row_length/2), -1), line_width, colors.black) # should depend on row_length
