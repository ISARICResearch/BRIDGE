...
# Function to generate the form as one pdf document.
def generate_pdf(data_dictionary, version, db_name,commit,isTest=False):
    if isinstance(db_name, list):
        db_name=db_name[0]

    # Change: Added an optional parameter "isTest" to just hold if we are running a test PDF
    ...
    # Change: Added this to be easy for me to test from my own file test.py.  
        # Generally not a test, and so will use buffer.seek(0) and return value
    if not isTest:
        buffer.seek(0)
...
# Function to generate seperate completion guide pdf document.
