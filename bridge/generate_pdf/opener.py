...
class ElementList:
    ...
    def add_design_description(self, element_list: list) -> list:
        ...
        design_text = (
            self.df_details.loc[
                self.df_details[PAPER_LIKE_FIELD] == match_text, self.text_field
            ]
            .values[0]
            .replace("[PROJECT_NAME]", self.db_name)
        )
        section_name = self.df_details.loc[
            self.df_details[PAPER_LIKE_FIELD] == match_text, self.paper_like_field
        ].values[0]
    ...
    def add_presentation_paragraphs(self, element_list: list) -> list:
        ...
        df_form_details = self.df_details[
            ...
            & self.df_details[PAPER_LIKE_FIELD].isin(
                [
                    "PRESENTATION FORM",
                    "DAILY FORM",
                    "OUTCOME FORM",
        ...
        element_list.append(
            Paragraph("<br/>".join(presentation_paragraphs), self.normal_style)
        )
        return element_list

    def add_follow_up_details(self, element_list: list) -> list:
        # Filtering out rows that are not related to form details
        follow_up_text = self.df_details.loc[
            self.df_details[PAPER_LIKE_FIELD] == "Follow-up details", self.text_field
        ].iloc[0]
    ...
    def get_timing_events_dataframe(self) -> pd.DataFrame:
        df_details_event = self.df_details[
            (self.df_details[TEXT_FIELD].str.startswith("Timing /Events:"))
            | (self.df_details[PAPER_LIKE_FIELD] == "Timing /Events")
        ].copy()
        ...
        timing_text = self.df_details.loc[
            self.df_details[PAPER_LIKE_FIELD] == "Timing /Events", self.paper_like_field
        ].iloc[0]
        df_transformed = df_transformed.loc[df_transformed["Forms"] != timing_text]
        return df_transformed

    def add_table_data(self, df_transformed: pd.DataFrame, element_list: list) -> list:
        # Convert DataFrame data into a list of lists with Paragraphs for wrapping
        table_data_list = [
            [Paragraph(str(item), self.center_style) for item in row]
            for row in df_transformed.values.tolist()
        ]
        header_list = [
            Paragraph(str(header), self.center_style)
            for header in df_transformed.columns.tolist()
        ]
        table_data_list.insert(0, header_list)

        # Calculate the available width for the table
        page_width = 8.5 * inch
        margin_width = 0.4 * inch  # Assuming a 1-inch margin on both sides
        table_width = (
            page_width - 2 * margin_width
        )  # subtracting both left and right margins
        num_columns = len(df_transformed.columns)
        col_width = table_width / num_columns

        table = Table(
            table_data_list, colWidths=[col_width for _ in range(num_columns)]
        )

        style = TableStyle(
            [
                ("BACKGROUND", (0, 0), (-1, 0), colors.hsl2rgb(0, 0, 0.7)),
                ...
                (
                    ...
                    (-1, -1),
                    8,
                ),  # Adjust the font size for all cells here
    ...
    def add_general_guidance(self, element_list: list) -> list:
        guidance_text = self.df_details.loc[
            self.df_details[PAPER_LIKE_FIELD] == "GENERAL GUIDANCE",
            self.paper_like_field,
        ].values[0]
        ...
        for entry in self.df_details.loc[
            self.df_details[PAPER_LIKE_FIELD] == "GENERAL GUIDANCE", self.text_field
        ].values:
            bullet_point = Paragraph(
                "• "
                + entry.replace("circles ()", "circles (○)").replace(
...
def set_styles() -> (
    Tuple[StyleSheet1, PropertySet, PropertySet, PropertySet, PropertySet]
):
...
def generate_opener(element_list: list, df_details: pd.DataFrame, db_name: str) -> list:
    (text_field, paper_like_field) = get_text_variables_for_env()

    (sample_style_sheet, normal_style, center_style, header_style, title_style) = (
        set_styles()
    )

    element_instance = ElementList(
        df_details,
        db_name,
        text_field,
        paper_like_field,
    ...
    element_list = element_instance.add_table_data(df_transformed, element_list)
