...
def on_modal_button_click(submit_n_clicks: int,
    ...
    if trigger_id == 'modal_submit':
        ...
        multilist_variable_choices_saved_list = json.loads(multilist_variable_choices_saved)
        ...
        if (variable_selected in ulist_variables) | (variable_selected in multilist_variables):
            ...
            tree_items = html.Div(
                dash_treeview_antd.TreeView(
                    ...
                    multiple=False,
                    checkable=True,
                    checked=df_datadicc['Variable'].loc[df_datadicc['Variable'].isin(checked)],
                    expanded=df_datadicc['Variable'].loc[df_datadicc['Variable'].isin(checked)],
...
def update_list_variables_checked(variable_choices_list: list,
                                  variable_name: str,
                                  df_checked: pd.DataFrame,
                                  df_current_datadicc: pd.DataFrame,
                                  other_text: str) -> Tuple[pd.DataFrame, str]:
        ...
        if var_select[0] == variable_name:
            ...
            for option_var_select in (var_select[1]):
                ...
                if list_item_name in (list(df_checked['Option'])):
                    ...
                    select_answer_options += str(list_item_number) + ', ' + str(list_item_name) + ' | '
                else:
                    list_options.append([list_item_number, list_item_name, 0])
                    not_select_answer_options += str(list_item_number) + ', ' + str(list_item_name) + ' | '

            variable_list_options.append([var_select, list_options])
            variable_choices_list[position][1] = variable_list_options[0][1]

            df_current_datadicc.loc[df_current_datadicc['Variable'] == variable_name, 'Answer Options'] = (
            ...
            if variable_name_not_selected in list(df_current_datadicc['Variable']):
                df_current_datadicc.loc[
                    df_current_datadicc['Variable'] == variable_name_not_selected, 'Answer Options'] = (
