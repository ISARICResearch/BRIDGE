    ...
    if filename:
        try:
            ...
            raise AttributeError(
                f"Failed to determine ARC version and/or language from file {filename}"
            )
        return (
...
def load_upload_arc_version_language(
...
def update_output_upload_crf(
    ...
    df_version_lang_datadicc = pd.read_json(
...
def update_list_variables_checked_upload(
    ...
    for _, row in df_list_saved.iterrows():
        ...
        df_upload_variable = df_list_upload.loc[
            df_list_upload["Variable"] == variable_name
        ]
        ...
        for list_item in list_items_saved:
            ...
            if not df_upload_variable.empty:
                ...
                if list_item_name in checked_list:
                    list_items_updated.append(
                        [list_item_number, str(list_item_name), 1]
                    )
                    select_answer_options += (
                    ...
                    list_items_updated.append(
                        [list_item_number, str(list_item_name), 0]
                    )
                    not_select_answer_options += (
                        f"{list_item_number}, {str(list_item_name)} | "
                    )
