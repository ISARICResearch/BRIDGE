...
def on_save_click(
    ...
    if trigger_id == "crf_save":
        ...
        filename_csv = (
            f"template_{crf_name}_{version_string}_{current_language}_{date}.csv"
        )

        df_current_datadicc = pd.read_json(
            io.StringIO(current_datadicc_saved), orient="split"
        )
        df_save = df_current_datadicc.loc[
            df_current_datadicc["Variable"].isin(checked_variables)
        ][["Variable"]]

        df_ulist_all = pd.DataFrame(
            json.loads(ulist_variable_choices_saved), columns=["Variable", "Ulist"]
        )
        df_multilist_all = pd.DataFrame(
            json.loads(multilist_variable_choices_saved),
            columns=["Variable", "Multilist"],
        )

        df_ulist_out = get_checked_data_for_list(
            df_ulist_all, checked_variables, "Ulist"
        )
        df_multilist_out = get_checked_data_for_list(
            df_multilist_all, checked_variables, "Multilist"
        )
...
def get_checked_data_for_list(
    df_list: pd.DataFrame, checked_variables: list, list_type: str
) -> pd.DataFrame:
