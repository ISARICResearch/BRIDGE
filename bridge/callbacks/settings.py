...
@dash.callback(
    Output("output-files-store", "data"), Input("output-files-checkboxes", "value")
)
...
def store_data_for_selected_version_language(
    n_clicks_version: list,
    ...
    try:
        (
            ...
            selected_version, selected_language, initial_load
...
def _determine_initial_load_boolean(n_clicks_version, n_clicks_language):
    ...
    n_clicks_language_unique_values = [
        click for click in n_clicks_language if pd.notnull(click)
    ]
