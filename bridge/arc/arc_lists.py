...
class ArcList:
    ...
    def _get_list_choices(self,
                          ...
                          other_text: str) -> Tuple[str, list]:
        df_list_options = ArcApiClient().get_dataframe_arc_list_version_language(self.version,
                                                                                 self.language,
                                                                                 str(datadicc_row['List']).replace(
                                                                                     '_', '/'))
    ...
    @staticmethod
    def _get_question_upper_lower(datadicc_row: pd.Series,
                                  question_english_lower: str) -> str:
    ...
    def _get_question_text_dropdown_row_list_content(self,
                                                     datadicc_row: pd.Series,
                                                     iteration_number: int,
                                                     select_text: str,
                                                     select_additional_text: str) -> str:
        ...
        if iteration_number == 0:
            ...
            if 'select' in question_english_lower:
                question_text = f'{ARROWS[iteration_number]}{question} {question_suffix}'
            else:
                question_text = f'{ARROWS[iteration_number]} {select_additional_text} {question} {question_suffix}'

        return question_text

    def _format_dropdown_row_list_content(self,
                                          datadicc_row: pd.Series,
                                          iteration_number: int,
                                          list_choices: str,
                                          select_text: str,
                                          select_additional_text: str) -> pd.Series:
        dropdown_row = datadicc_row.copy()
        dropdown_row['Variable'] = f'{datadicc_row['Sec']}_{datadicc_row['vari']}_{str(iteration_number)}item'
        ...
        if iteration_number == 0:
            ...
            dropdown_row[
                'Skip Logic'] = f"[{datadicc_row['Sec']}_{datadicc_row['vari']}_{str(iteration_number - 1)}addi]='1'"
        return dropdown_row

    def _get_question_text_other_row_list_content(self,
                                                  datadicc_row: pd.Series,
                                                  iteration_number: int,
                                                  specify_text: str,
                                                  specify_other_text: str,
                                                  specify_other_infection_text: str) -> str:
        if datadicc_row['Variable'] == 'inclu_disease':
            ...
            if iteration_number == 0:
                if 'other' in question_english_lower:
                    question_text = f'{ARROWS[iteration_number]} {specify_text} {question}'
                else:
                    question_text = f"{ARROWS[iteration_number]} {specify_other_text} {question}"
    ...
    def _format_other_row_list_content(self,
                                       datadicc_row: pd.Series,
                                       iteration_number: int,
                                       dropdown_row: pd.Series,
                                       specify_text: str,
                                       specify_other_text: str,
                                       specify_other_infection_text: str) -> pd.Series:
        other_row = datadicc_row.copy()
        other_row['Variable'] = f'{datadicc_row['Sec']}_{datadicc_row['vari']}_{str(iteration_number)}otherl2'
    ...
    @staticmethod
    def _append_updated_other_info_list_content(datadicc_row: pd.Series,
                                                iteration_number: int,
                                                other_info: pd.DataFrame,
                                                questions_for_this_list: list) -> List:
        ...
        if len(other_info) > 1:
            for index, other_info_row in other_info.iterrows():
                ...
                if iteration_number == 0:
                    question_text = f'{ARROWS[iteration_number]} {datadicc_row['Question']}'
                else:
                ...
                other_info_row_updated[
                    'Variable'] = f'{datadicc_row['Sec']} {datadicc_row['vari']}_{str(iteration_number)}{datadicc_row['mod']}'
                other_info_row_updated['Skip Logic'] = f"[{skip_logic_variable}]='1'"
                other_info_row_updated['List'] = None
                other_info_row_updated['mod'] = f'{str(iteration_number)}{datadicc_row['mod']}'
            ...
            other_info_row_updated = other_info_row.copy()
            if iteration_number == 0:
                other_info_row_updated['Question'] = f'{ARROWS[iteration_number]}{other_info_row['Question']}'
            else:
                question_text = f'{ARROWS[iteration_number]}{other_info_row['Question']} {str(iteration_number + 1)}'
                other_info_row_updated['Question'] = question_text
                other_info_row_updated['Skip Logic'] = f"[{skip_logic_variable}]='1'"
                other_info_row_updated[
                    'Variable'] = f'{other_info_row['Sec']}_{other_info_row['vari']}_{str(iteration_number)}{other_info_row['mod']}'
                other_info_row_updated['List'] = None
                other_info_row_updated['mod'] = f'{str(iteration_number)}{other_info_row['mod']}'
                other_info_row_updated['vari'] = other_info_row['vari']
            questions_for_this_list.append(other_info_row_updated)

        return questions_for_this_list

    def _append_updated_additional_row_list_content(self,
                                                    datadicc_row: pd.Series,
                                                    iteration_number: int,
                                                    iteration_total: int,
                                                    dropdown_row: pd.Series,
        ...
        if iteration_number < iteration_total - 1:
            ...
            additional_row[
                'Variable'] = f'{datadicc_row['Sec']}_{datadicc_row['vari']}_{str(iteration_number)}addi'
            ...
            question_english_lower = str(datadicc_row['Question_english'].lower())
            question = self._get_question_upper_lower(datadicc_row, question_english_lower)
            question_text = f"{ARROWS[iteration_number]} {any_additional_text} {question} ?"
    ...
    def get_list_content(self,
                         df_datadicc: pd.DataFrame) -> tuple[pd.DataFrame, list]:
        all_rows_lists = []
        ...
        for _, datadicc_row in df_datadicc_lists.iterrows():
            if pd.isnull(datadicc_row['List']):
                ...
                (list_choices,
                 list_variable_choices_aux) = self._get_list_choices(datadicc_row,
                                                                     other_text)
                ...
                for iteration_number in range(iteration_total):
                    dropdown_row = self._format_dropdown_row_list_content(datadicc_row,
                                                                          iteration_number,
                                                                          list_choices,
                                                                          select_text,
                                                                          select_additional_text)

                    other_row = self._format_other_row_list_content(datadicc_row,
                                                                    iteration_number,
                                                                    dropdown_row,
                                                                    specify_text,
                                                                    specify_other_text,
                    ...
                    other_info = df_datadicc.loc[(df_datadicc['Sec'] == datadicc_row['Sec']) &
                                                 (df_datadicc['vari'] == datadicc_row['vari']) &
                                                 (df_datadicc['Variable'] != datadicc_row['Variable'])]

                    questions_for_this_list = self._append_updated_other_info_list_content(datadicc_row,
                                                                                           iteration_number,
                                                                                           other_info,
                                                                                           questions_for_this_list)

                    questions_for_this_list = self._append_updated_additional_row_list_content(datadicc_row,
                                                                                               iteration_number,
                                                                                               iteration_total,
                                                                                               dropdown_row,
                                                                                               any_additional_text,
                ...
                list_variable_choices.append([datadicc_row['Variable'], list_variable_choices_aux])
    ...
    @staticmethod
    def _get_list_option_number(df_list_options: pd.DataFrame,
                                list_option: str) -> int:
        if 'Value' in df_list_options.columns:
            list_option_number = int(
                df_list_options['Value'].loc[df_list_options[df_list_options.columns[0]] == list_option].iloc[0])
        else:
            # fallback to index-based counting
            list_option_number = df_list_options[df_list_options.columns[0]].values.tolist().index(list_option) + 1
    ...
    @staticmethod
    def _get_list_data_dropdown_other_rows(row: pd.Series,
                                           translations_for_language: dict,
                                           list_type: str,
                                           l2_choices: str) -> Tuple[pd.Series, pd.Series]:
    ...
    def _get_list_data(self, df_datadicc: pd.DataFrame,
                       list_type: str) -> tuple[list, list]:
        all_rows_lists = []
        list_variable_choices = []
        ...
        for _, row in datadicc_disease_lists.iterrows():
            if pd.isnull(row['List']):
                ...
                list_variable_choices_aux = []
                for list_option in df_list_options[df_list_options.columns[0]]:
                    list_option_number = self._get_list_option_number(df_list_options, str(list_option))
                    try:
                        df_list_options['Selected'] = pd.to_numeric(df_list_options['Selected'], errors='coerce')

                        selected_value = df_list_options['Selected'].loc[
                            df_list_options[df_list_options.columns[0]] == list_option].iloc[0]
                ...
                (dropdown_row,
                 other_row) = self._get_list_data_dropdown_other_rows(row,
    ...
    def get_user_list_content(self,
                              df_datadicc: pd.DataFrame) -> tuple[pd.DataFrame, list]:
        ulist_variable_choices, all_rows_lists = self._get_list_data(df_datadicc,
                                                                     'user_list')
        df_arc_list = pd.DataFrame(all_rows_lists).reset_index(drop=True)
        return df_arc_list, ulist_variable_choices

    def get_multi_list_content(self,
                               df_datadicc: pd.DataFrame) -> tuple[pd.DataFrame, list]:
        multilist_variable_choices, all_rows_lists = self._get_list_data(df_datadicc,
                                                                         'multi_list')
        df_arc_list = pd.DataFrame(all_rows_lists).reset_index(drop=True)
