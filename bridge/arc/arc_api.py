...
class ArcApiClient:
    ...
    def get_arc_version_list(self) -> list:
        if self.environment != 'development':
            ...
            release_json = self._get_api_response(url)
            version_list = sorted([self.get_version_string(release_dict['name']) for release_dict in release_json if
                                   release_dict['name'].startswith('ARCH')], reverse=True)
        return version_list
    ...
    def get_dataframe_arc_sha(self,
        ...
        if self.environment != 'development':
            ...
            url = '/'.join(
                [self.base_url_raw_content, 'DataPlatform', sha, 'ARCH', self.get_arch_version_string(version),
                 'ARCH.csv'])
    ...
    def get_dataframe_arc_version_language(self,
        ...
        if self.environment != 'development':
            ...
            url = '/'.join(
                [self.base_url_raw_content, 'DataPlatform', 'main', 'ARCH', self.get_arch_version_string(version),
                 'ARCH.csv'])
        df = self._write_to_dataframe(url)
        return df
    ...
    def get_dataframe_paper_like_details(self,
        ...
        if self.environment != 'development':
            url = '/'.join(
                [self.base_url_raw_content, 'ARC-Translations', 'main', self.get_arch_version_string(version),
    ...
    def get_dataframe_supplemental_phrases(self,
        ...
        if self.environment != 'development':
            ...
            version_list = [release_dict['tag_name'] for release_dict in release_json]
            url = '/'.join(
                [self.base_url_raw_content, 'ARC-Translations', 'main', self.get_arch_version_string(max(version_list)),
                 language, 'supplemental_phrases.csv'])
        df = self._write_to_dataframe(url)
        return df
